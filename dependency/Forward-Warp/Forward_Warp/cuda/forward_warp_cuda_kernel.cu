...
// Ajusta si usas torch::native::detail::GridSamplerInterpolation
...
__global__ void forward_warp_cuda_forward_kernel(
  ...
  CUDA_KERNEL_LOOP(index, total_step) {
    ...
    const scalar_t x = static_cast<scalar_t>(w) + flow[index * 2 + 0];
    const scalar_t y = static_cast<scalar_t>(h) + flow[index * 2 + 1];
  ...
  CUDA_KERNEL_LOOP(index, total_step) {
    ...
    if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
      ...
      if (x_f >= 0 && x_c < W && y_f >= 0 && y_c < H) {
        ...
        // Puntero para el gradiente de im0
        ...
        for (int c = 0; c < C; ++c, im0_grad_p += H*W) {
          ...
          // Calcula gradiente con respecto a flow (x, y)
...
// -----------------------------------------------------------------------------
//  Ejemplo de funciones "max_motion" o similares (omitidas si no las usas).
// -----------------------------------------------------------------------------

template <typename scalar_t>
static __forceinline__ __device__
void find_max_motion(const int d, const scalar_t wght, int* d_buffer_p) {
  if (wght >= 0.25) {
...
__global__ void forward_warp_max_motion_cuda_forward_kernel(
  ...
  // Aquí tu lógica de "max motion"...
  // (Se sustituirían data<scalar_t>() por data_ptr<scalar_t>() en la función que la llame)
  // ...
}

// -----------------------------------------------------------------------------
//  Funciones wrapper de la API C++
// -----------------------------------------------------------------------------
...
at::Tensor forward_warp_max_motion_cuda_forward(
  ...
  // Usa im0.scalar_type() en vez de im0.type()
...
at::Tensor forward_warp_cuda_forward(
  ...
  // Creamos im1 en ceros, del mismo tamaño
